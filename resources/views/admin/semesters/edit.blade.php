        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Edit Semester: {{ $semester->name }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.semesters.update', $semester) }}" method="POST" id="semesterForm">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('term_number') is-invalid @enderror" 
                                        ...
                                        <option value="1" {{ old('term_number', $semester->term_number) == '1' ? 'selected' : '' }}>1st Term</option>
                                        <option value="2" {{ old('term_number', $semester->term_number) == '2' ? 'selected' : '' }}>2nd Term</option>
                                        <option value="3" {{ old('term_number', $semester->term_number) == '3' ? 'selected' : '' }}>3rd Term</option>
                                        <option value="summer" {{ old('term_number', $semester->term_number) == 'summer' ? 'selected' : '' }}>Summer Term</option>
                                    </select>
                                    @error('term_number')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control @error('enrollment_end') is-invalid @enderror" 
                                           ...
                                           value="{{ old('enrollment_end', $semester->enrollment_end?->format('Y-m-d')) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="Optional description for this semester">{{ old('description', $semester->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="form-check">
                                ...
                                <small class="form-text text-muted d-block">
                                    @if($semester->is_current)
                                        This semester is currently set as the active semester
                                    @else
                                        This will unset any existing current semester
                                    @endif
                                </small>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <a href="{{ route('admin.semesters.show', $semester) }}" class="btn btn-outline-info">
                                    <i class="bi bi-eye"></i> View Details
                                </a>
                            </div>
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-check"></i> Update Semester
                            </button>
                        </div>
                    </form>
                </div>
    ...
    function generateSemesterName() {
        ...
        if (academicYear && academicYear !== 'Select Academic Year' && termNumber) {
            ...
            if (termName) {
                ...
                // Only auto-generate if the current name follows the pattern or is empty
                const currentName = $('#name').val();
                if (!currentName || currentName.includes('Semester') || currentName.includes('Term')) {
